...
export default function EditComicPage() {
    ...
    // Fetch genres & comic data
    useEffect(() => {
        const loadInitialData = async () => {
            try {
                // Fetch genres
                const genresRes = await adminService.getGenres();
                setGenres(genresRes.data || []);
                setLoadingGenres(false);

                // Fetch comic detail
                const comicData = await adminService.getComic(params.id);
                if (comicData) {
                    setFormData({
                        title: comicData.title || '',
                        alternative_title: comicData.alternative_title || '',
                        ...
                        original_language: comicData.original_language || 'ja',
                        release_year: comicData.release_year || new Date().getFullYear(),
                        is_featured: !!comicData.is_featured,
                        is_published: !!comicData.is_published,
                        genres: comicData.genres?.map(g => g.id) || [],
                    });
                    
                    if (comicData.cover_image_url || comicData.cover_image) {
                        setCoverPreview(comicData.cover_image_url || comicData.cover_image);
                    }
                    if (comicData.banner_image_url || comicData.banner_image) {
                        setBannerPreview(comicData.banner_image_url || comicData.banner_image);
                    }
                }
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            submitData.append('_method', 'PUT'); // For Laravel resourceful mapping
            ...
            await adminService.updateComic(params.id, submitData);
    ...
    if (fetching) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="text-[var(--text-secondary)]">Memuat data komik...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-[var(--text-primary)]">Edit Komik</h1>
                    <p className="text-[var(--text-secondary)]">Perbarui informasi komik</p>
                </div>
            </div>
